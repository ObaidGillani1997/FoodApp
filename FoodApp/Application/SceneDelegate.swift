...
//  FoodApp
//
//  Created by Obaid Khan on 04/04/22.
//
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene,
        ...
        showRootViewController(navigationController: navigationController)
        window.rootViewController = navigationController
        window.makeKeyAndVisible()
        self.window = window
    }

    func showRootViewController(navigationController: UINavigationController) {
        let sessionManager = Alamofire.Session()
        let service = FoodService(sessionManager: sessionManager, api: .food)
        let foodViewModel = FoodViewModel(service: service)
